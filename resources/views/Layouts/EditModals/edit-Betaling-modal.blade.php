<!-- ===== Edit Betaling Modal ===== -->
<div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[1000] opacity-0 invisible transition-all duration-300 [&.active]:opacity-100 [&.active]:visible" id="editBetalingModal">
    <div class="transform translate-y-5 scale-[0.97] opacity-0 transition-all duration-[350ms] [.active_&]:translate-y-0 [.active_&]:scale-100 [.active_&]:opacity-100 w-full max-w-[640px] max-h-[90vh] overflow-y-auto m-4 scrollbar-thin">
        <div class="bg-white rounded-2xl p-8 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.2),0_0_0_1px_rgba(226,232,240,0.5)]">
            ...
            <div class="flex justify-between items-start mb-6">
                <div>
                    <h2 class="text-xl font-bold text-slate-800 tracking-tight m-0">Bewerk Betaling</h2>
                    <p class="text-[0.8125rem] text-slate-400 mt-1">Pas de gegevens aan om de betaling bij te werken</p>
                </div>
                <button type="button" id="closeEditBetalingModalBtn" aria-label="Sluiten"
            ...
            {{-- Section title --}}
            <div class="text-[0.6875rem] font-bold text-slate-400 mb-5 uppercase tracking-widest">Wijzig Details</div>

            {{-- Error container --}}
            <div class="bg-red-50 text-red-700 p-3.5 rounded-[10px] mb-5 border-l-4 border-red-500 animate-[shakeError_0.4s_ease] hidden" id="editBetalingModalErrors">
                <ul class="m-0 pl-5 text-[0.8125rem]" id="editBetalingModalErrorList"></ul>
            </div>

            <form id="editBetalingForm" method="POST">
                @csrf
                @method('PATCH')

                <input type="hidden" id="edit_betaling_id" name="id">

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 gap-x-7 mb-6">

                    {{-- Naam (Read Only) --}}
                    <div class="flex flex-col">
                        <label for="edit_betaling_naam" class="text-[0.8125rem] font-semibold text-slate-700 mb-1.5">
                            Naam
                        </label>
                        <input type="text" id="edit_betaling_naam" name="naam" disabled
                            class="px-3 py-2.5 border-[1.5px] border-slate-200 rounded-[10px] text-[0.8125rem] text-slate-400 bg-slate-100 w-full box-border font-[inherit] cursor-not-allowed">
                    </div>

                    {{-- Datum van betaling (Read Only) --}}
                    <div class="flex flex-col">
                        <label for="edit_betaling_datum" class="text-[0.8125rem] font-semibold text-slate-700 mb-1.5">
                            Datum van betaling
                        </label>
                        <input type="date" id="edit_betaling_datum" name="datum" readonly
                            class="px-3 py-2.5 border-[1.5px] border-slate-200 rounded-[10px] text-[0.8125rem] text-slate-400 bg-slate-100 w-full box-border font-[inherit] cursor-not-allowed">
                    </div>

                    {{-- Methode --}}
                    <div class="flex flex-col">
                        <label for="edit_betaling_methode" class="text-[0.8125rem] font-semibold text-slate-700 mb-1.5">
                            Methode <span class="text-red-500">*</span>
                        </label>
                        <select id="edit_betaling_methode" name="methode" required
                    ...
                    <div class="flex flex-col" id="editBetalingBewijsContainer" style="display: none;">
                        <label for="edit_betaling_bewijs" class="text-[0.8125rem] font-semibold text-slate-700 mb-1.5">
                            Betaling bewijs
                        </label>
                        <div class="relative">
                            <input type="file" id="edit_betaling_bewijs" name="betaling_bewijs" accept="image/*,.pdf"
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-[2]">
                            <div id="editFileUploadBtn"
                                ...
                                <span id="editFileUploadText">Bestand uploaden</span>
                            </div>
                        </div>
                    </div>
...
{{-- Status --}}
                    ...
                    {{-- Bedrag --}}
                    <div class="flex flex-col">
                        <label for="edit_betaling_bedrag" class="text-[0.8125rem] font-semibold text-slate-700 mb-1.5">
                        ...
                        <div class="flex items-stretch border-[1.5px] border-slate-200 rounded-[10px] overflow-hidden bg-slate-50 transition-all duration-200 focus-within:border-blue-600 focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(37,99,235,0.08)]">
                            ...
                            <input type="number" id="edit_betaling_bedrag" name="bedrag" placeholder="0.00" step="0.01" min="150" required
                ...
                <div class="flex gap-3 mt-6 pt-6 border-t border-slate-100">
                    <button type="submit" id="submitEditBetalingBtn"
                    ...
                    <button type="button" id="cancelEditBetalingModalBtn"
